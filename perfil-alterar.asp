...
<head>
  ...
  <script language="JavaScript">
    ...
    function lancarPerfil() 
    {
      if(confirm('Deseja finalizar essa operação?'))
        document.form.action = "perfil-grava.asp";
      else
        return false;
    }
    </script>
...
<div class="wrapper">
  ...
  sqlstmt = "select * from perfil where codigo=" & request("pid")
            ...
            <div class="card-body">
              <div class="col-md-12">
                <div class="row">
                  <div class="col-md-3">
                     <label>Perfil</label>
                    <input  type="text" oninput="this.value = this.value.toUpperCase() " class="rcbfocus form-control" placeholder="Digite o perfil" name="perfil" value="<%response.write rsquery("perfil")%>" />
                  </div>
                  <div class="col-md-1" style="top: 30px;"  align="right">
                      <button onclick="lancarPerfil();" class="btn btn-info">Alterar</button>
                  </div>
                </div>
              </div>
            </div>
